# Weekly Indices Review 2026-05-05

> *This report is for informational and educational purposes only; please see the disclaimer at the end.*

## 1. Executive Summary
- Primary regime: Policy Transition / Mixed Regime
- Geopolitical regime: Elevated but localized
- Main takeaway: Manifest-backed production rerun of the upgraded ETF-derived Weekly Index workflow: fresh closing prices first, full-universe breadth, long opportunities, short opportunities radar, capital re-underwriting scorecard, render validation, and email delivery.
- This file is a same-day production trigger scaffold. The workflow must replace artifact-driven sections before render/send and then commit a run manifest under `output_indices/run_manifests/`.

## 2. Portfolio Action Snapshot
### Add
- None before live pricing and ranking rebuild.
### Hold
- S&P 500 (**SPY**)
- Nasdaq 100 (**QQQ**)
- Russell 2000 (**IWM**)
- Emerging Markets (**EEM**)
### Hold but replaceable
- Russell 2000 (**IWM**)
- Emerging Markets (**EEM**)
### Reduce
- None before live pricing and ranking rebuild.
### Close
- None before live pricing and ranking rebuild.
### Best replacements to fund
- To be rebuilt from live candidate-ranking artifacts.
### Top 3 actions this week
1. Validate fresh closing prices and current FX basis first.
2. Rebuild full-universe breadth, long opportunities, and short opportunities radar.
3. Validate capital re-underwriting scorecard, render, email, and manifest commit-back.
### Top 3 risks this week
1. Pricing coverage or FX freshness fails the hardened pricing gate.
2. Weak holdings remain vague instead of producing alternatives, triggers, or override reasons.
3. Render or email delivery fails without a positive manifest/receipt.

## 3. Global Regime Dashboard
- Primary regime remains mixed / transitionary until live macro artifacts confirm otherwise.
- Geopolitical regime remains elevated but localized until current research artifacts confirm otherwise.
- Portfolio implication: stay selective, preserve breadth discipline, and keep defensive / inverse opportunities separate from the long-side board.

## 4. Index Opportunity Board

| Exposure | Benchmark / public index | Implementation proxy | Regional group | Score | Status | Why it is on the board |
|---|---|---|---|---:|---|---|
| Emerging Markets | Emerging Markets | EEM | EM broad | 3.08 | Funded | Emerging markets add a measured non-U.S. risk sleeve while the dollar backdrop is less hostile. |
| Nasdaq 100 | Nasdaq 100 | QQQ | U.S. core leadership | 2.89 | Funded | Growth leadership remains a core engine in the current opportunity set. |
| Russell 2000 | Russell 2000 | IWM | U.S. core leadership | 2.72 | Funded | Domestic breadth improves diversification without dominating the book. |
| S&P 500 | S&P 500 | SPY | U.S. core leadership | 2.61 | Funded | Core U.S. large-cap exposure remains the cleanest starting anchor. |
| Nikkei 225 | Nikkei 225 | EWJ | developed Asia-Pacific | 2.57 | Surfaced | Improves breadth or fills an important portfolio gap without forcing a low-conviction rotation. |

The board remains intentionally compact. The strongest omitted regional challenger this run is **S&P/TSX 60 (EWC)**, which remains close enough to matter without displacing a higher-ranked funded exposure.

## 5. Key Risks / Invalidators
- Pricing coverage fails the fresh-close threshold.
- FX reference data is stale versus the requested close date.
- Breadth deteriorates enough to weaken small caps or EM.
- U.S. mega-cap concentration remains high without sufficient breadth confirmation.
- Render, PDF, manifest, scorecard, or email delivery fails.

## 6. Bottom Line
- Fresh pricing comes first.
- The long-side opportunity board and short opportunities radar must both be visible.
- Weak or replaceable holdings require named alternatives, triggers, or explicit override reasons.
- No delivery success should be claimed without workflow evidence or manifest evidence.

## 7. Equity Curve and Portfolio Development

- Starting capital (EUR): 100000.00
- Current portfolio value (EUR): 108478.51
- Since inception return (%): 8.48
- Equity-curve state: Live tracked
- Pricing basis requested close date: 2026-05-04
- FX reference date: 2026-05-04
- Notes: Holdings and NAV in this section are rebuilt from the live pricing/state layer for the requested close date 2026-05-04.

| Date | Portfolio value (EUR) | Comment |
|---|---:|---|
| 2026-04-27 | 105772.19 | Pricing basis close 2026-04-27 |
| 2026-04-27 | 105772.19 | Pricing basis close 2026-04-27 |
| 2026-04-27 | 105772.19 | Pricing basis close 2026-04-27 |
| 2026-04-27 | 105772.19 | Pricing basis close 2026-04-27 |
| 2026-04-30 | 107173.81 | Pricing basis close 2026-04-30 |
| 2026-05-01 | 107643.74 | Pricing basis close 2026-05-01 |
| 2026-05-04 | 107535.26 | Pricing basis close 2026-05-04 |
| 2026-05-04 | 107535.26 | Pricing basis close 2026-05-04 |
| 2026-05-04 | 108478.51 | Pricing basis close 2026-05-04 |
| 2026-05-04 | 108478.51 | Pricing basis close 2026-05-04 |

`EQUITY_CURVE_CHART_PLACEHOLDER`

## 8. Regional / Style Allocation Map
- U.S. core leadership remains the funded anchor before the live rebuild.
- Europe, UK, Switzerland, Japan, Canada, Australia, Greater China, India, and EM broad must stay visible in the breadth checkpoint.
- Defensive / inverse opportunities are tactical hedge candidates, not base-case allocations.

## 9. Second-Order Effects Map
- Rates, credit, oil, dollar pressure, and market breadth determine whether higher-beta exposures deserve more or less capital.
- If breadth improves, long-side challengers can move closer to funding.
- If breadth breaks, inverse small-cap, Nasdaq, S&P 500, and EM/developed ex-U.S. hedge lanes deserve elevation.

## 10. Current Position Review
### S&P 500 / SPY
- Would initiate today: pending live pricing and ranking rebuild.
- Would initiate at current weight: pending SPY / QQQ overlap review.
- Best alternative: VOO or broader non-U.S. challenger.
- Required next action: test U.S. concentration and breadth quality.

### Nasdaq 100 / QQQ
- Would initiate today: pending live pricing and ranking rebuild.
- Would initiate at current weight: pending mega-cap leadership review.
- Best alternative: QQQM or lower-beta quality challenger.
- Required next action: test whether growth leadership still deserves top weight.

### Russell 2000 / IWM
- Would initiate today: pending breadth confirmation.
- Would initiate at current weight: no if breadth deteriorates.
- Best alternative: VTWO if constructive; RWM if deterioration triggers.
- Required next action: direct breadth validity and RWM defensive comparison.

### Emerging Markets / EEM
- Would initiate today: pending dollar and EM breadth confirmation.
- Would initiate at current weight: no if dollar pressure reaccelerates.
- Best alternative: VWO / INDA if constructive; EUM if deterioration triggers.
- Required next action: direct EM validity and EUM defensive comparison.

## 11. Best New Index Opportunities

### Long-side Opportunities

The strongest omitted regional challenger this run is **China large cap (FXI)**. It improves breadth and remains close enough to the live board to stay visible in the report.

#### 1. China large cap (FXI)

- Regional group: Greater China
- Challenger score: 2.09
- Why it matters: Ranks well internally but remains just below the current publication cutoff.
- Why not on the board yet: strong_challenger_not_published

#### 2. S&P/TSX 60 (EWC)

- Regional group: North America ex-U.S.
- Challenger score: 1.85
- Why it matters: Ranks well internally but remains just below the current publication cutoff.
- Why not on the board yet: strong_challenger_not_published

#### 3. FTSE MIB (EWI)

- Regional group: continental Europe
- Challenger score: 1.52
- Why it matters: Ranks well internally but remains just below the current publication cutoff.
- Why not on the board yet: board_capacity

### Best Defensive / Inverse Opportunities

#### 1. Short Russell 2000 via RWM

- Why it matters: the Russell 2000 remains the weakest current held sleeve and is the cleanest bearish expression if higher oil, tighter conditions, and funding stress keep hurting small caps.
- Trigger: further relative breakdown in small-cap breadth.
- Invalidation: clear improvement in small-cap relative strength and easing inflation pressure.

#### 2. Short Nasdaq 100 via PSQ

- Why it matters: this is the best pure hedge if U.S. quality leadership finally breaks under higher oil, higher yields, or earnings disappointment.
- Trigger: clear loss of Nasdaq 100 relative leadership or broad de-risking led by large-cap tech.
- Invalidation: renewed earnings-led strength in mega-cap growth and a calmer oil/policy backdrop.

#### 3. Short S&P 500 via SH

- Why it matters: this is the broad-market hedge if the current selective resilience turns into a wider U.S. risk-off move.
- Trigger: oil and policy stress begin to hit the broader index rather than just the weakest sleeves.
- Invalidation: continued S&P 500 earnings resilience and broad participation in upside.

#### 4. Short developed ex-U.S. via EFZ or short Emerging Markets via EUM

- Why it matters: these are the cleaner non-U.S. hedge lanes if Europe or broader emerging markets deteriorate faster than expected.
- Trigger: renewed energy stress, worsening Europe data, or broad EM weakness.
- Invalidation: oil stabilizes, Europe sentiment improves, and EM breadth strengthens.

**Important note:** these inverse instruments are defensive tools, not the base-case allocation. They are most appropriate as tactical hedges or bearish expressions under deterioration, not as default long-term holdings.

### Breadth checkpoint by regional bucket
| Regional bucket | Strongest candidate | Proxy | Challenger score | Current status |
|---|---|---|---:|---|
| U.S. core leadership | Nasdaq 100 | QQQ | 2.09 | Published |
| continental Europe | FTSE MIB | EWI | 1.52 | Ruled out / lower priority |
| UK | FTSE 100 | EWU | 1.40 | Ruled out / lower priority |
| Switzerland | SMI | EWL | 0.94 | Ruled out / lower priority |
| North America ex-U.S. | S&P/TSX 60 | EWC | 1.85 | Near miss |
| developed Asia-Pacific | Nikkei 225 | EWJ | 2.70 | Published |
| Greater China | China large cap | FXI | 2.09 | Near miss |
| India | Nifty 50 | INDA | 0.98 | Ruled out / lower priority |
| EM broad | Emerging Markets | EEM | 2.43 | Published |

### Universe scan checkpoint
| Exposure | Regional group | Proxy | Published? | Challenger score | Why not on the board yet |
|---|---|---|---|---:|---|
| Nikkei 225 | developed Asia-Pacific | EWJ | Yes | 2.70 | published |
| Emerging Markets | EM broad | EEM | Yes | 2.43 | published |
| China large cap | Greater China | FXI | No | 2.09 | strong_challenger_not_published |
| Nasdaq 100 | U.S. core leadership | QQQ | Yes | 2.09 | published |
| Russell 2000 | U.S. core leadership | IWM | Yes | 2.02 | published |
| S&P/TSX 60 | North America ex-U.S. | EWC | No | 1.85 | strong_challenger_not_published |
| S&P 500 | U.S. core leadership | SPY | Yes | 1.81 | published |
| FTSE MIB | continental Europe | EWI | No | 1.52 | board_capacity |
| Hang Seng | Greater China | EWH | No | 1.43 | weak_relative_strength |
| AEX | continental Europe | EWN | No | 1.41 | board_capacity |
| FTSE 100 | UK | EWU | No | 1.40 | board_capacity |
| DAX | continental Europe | EWG | No | 1.32 | board_capacity |
| ASX 200 | developed Asia-Pacific | EWA | No | 1.19 | weak_relative_strength |
| Nifty 50 | India | INDA | No | 0.98 | weak_relative_strength |
| SMI | Switzerland | EWL | No | 0.94 | weak_relative_strength |
| IBEX 35 | continental Europe | EWP | No | 0.86 | weak_relative_strength |
| Euro Stoxx 50 | continental Europe | FEZ | No | 0.52 | weak_relative_strength |
| CAC 40 | continental Europe | EWQ | No | 0.41 | weak_relative_strength |

## 12. Portfolio Rotation Plan
- Maintain current holdings unless rebuilt ranking artifacts justify a better funded board.
- Use whole shares only.
- Keep residual as cash.
- Do not deploy inverse instruments unless tactical deterioration triggers are met.

## 13. Final Action Table
| Ticker | Public index / exposure | Existing/New | Target Weight | Suggested Action | Conviction Tier | Total Score | Portfolio Role | Better Alternative Exists? | Short Reason |
|---|---|---|---:|---|---|---:|---|---|---|
| SPY | S&P 500 | Existing | TBD | Hold pending artifact rebuild | Tier 1 | TBD | Core beta | TBD | Core anchor; test overlap with QQQ |
| QQQ | Nasdaq 100 | Existing | TBD | Hold pending artifact rebuild | Tier 1 | TBD | Growth engine | TBD | Growth leadership; test concentration |
| IWM | Russell 2000 | Existing | TBD | Hold / replaceable pending artifact rebuild | Tier 2 | TBD | Breadth diversifier | RWM if deterioration triggers | Breadth sleeve under review |
| EEM | Emerging Markets | Existing | TBD | Hold / replaceable pending artifact rebuild | Tier 2 | TBD | Non-U.S. risk sleeve | EUM if deterioration triggers | EM sleeve under dollar-pressure review |
| CASH | Residual cash | Existing | TBD | Hold | Tier 1 | — | Optionality | — | Dry powder; classify cash policy |

## 14. Position Changes Executed This Run
- None before workflow artifact rebuild.
- Actual position changes must come from the production ranking/state layer.

## 15. Current Portfolio Holdings and Cash

- Starting capital (EUR): 100000.00
- Invested market value (EUR): 92911.77
- Cash (EUR): 15566.74
- Total portfolio value (EUR): 108478.51
- Since inception return (%): 8.48
- Pricing basis requested close date: 2026-05-04
- FX reference date: 2026-05-04

| Ticker | Public index / exposure | Shares | Price (local) | Currency | Market value (local) | Market value (EUR) | Weight % |
|---|---|---:|---:|---|---:|---:|---:|
| SPY | S&P 500 | 44 | 718.01 | USD | 31592.44 | 27002.09 | 24.89 |
| QQQ | Nasdaq 100 | 48 | 672.88 | USD | 32298.24 | 27605.33 | 25.45 |
| IWM | Russell 2000 | 90 | 277.88 | USD | 25009.20 | 21375.38 | 19.70 |
| EEM | Emerging Markets | 309 | 64.10 | USD | 19806.90 | 16928.97 | 15.61 |
| CASH | Residual cash | - | 1.00 | EUR | 15566.74 | 15566.74 | 14.35 |

## 16. Continuity Input for Next Run

### Watchlist / dynamic radar memory
| Theme | Regional group | Primary Proxy | Status | Why it stays visible |
|---|---|---|---|---|
| China large cap | Greater China | FXI | Strong challenger | Broad discovery keeps it visible even though it did not make the compact board. |
| S&P/TSX 60 | North America ex-U.S. | EWC | Strong challenger | Broad discovery keeps it visible even though it did not make the compact board. |
| FTSE MIB | continental Europe | EWI | Watchlist | Broad discovery keeps it visible even though it did not make the compact board. |
| Hang Seng | Greater China | EWH | Watchlist | Broad discovery keeps it visible even though it did not make the compact board. |
| AEX | continental Europe | EWN | Watchlist | Broad discovery keeps it visible even though it did not make the compact board. |
| FTSE 100 | UK | EWU | Watchlist | Broad discovery keeps it visible even though it did not make the compact board. |
| DAX | continental Europe | EWG | Watchlist | Improves breadth or fills an important portfolio gap without forcing a low-conviction rotation. |
| ASX 200 | developed Asia-Pacific | EWA | Watchlist | Broad discovery keeps it visible even though it did not make the compact board. |

### Discovery coverage checkpoint
| Regional group | Status | Strongest candidate | Proxy | Score |
|---|---|---|---|---:|
| U.S. core leadership | surfaced | Nasdaq 100 | QQQ | 2.09 |
| continental Europe | considered | FTSE MIB | EWI | 1.52 |
| UK | ruled_out | FTSE 100 | EWU | 1.40 |
| Switzerland | ruled_out | SMI | EWL | 0.94 |
| North America ex-U.S. | near_miss | S&P/TSX 60 | EWC | 1.85 |
| developed Asia-Pacific | surfaced | Nikkei 225 | EWJ | 2.70 |
| Greater China | near_miss | China large cap | FXI | 2.09 |
| India | ruled_out | Nifty 50 | INDA | 0.98 |
| EM broad | surfaced | Emerging Markets | EEM | 2.43 |

### Lane continuity notes
- Retained entries: none
- New entries: S&P 500, Nasdaq 100, Russell 2000, Emerging Markets broad
- Dropped entries: none
- Strong challengers not published: Japan large-cap equities, Germany cyclical equities, Europe broad large-cap equities
- What would most likely change the board next run: Cleaner confirmation in Japan or Europe broadens the funded opportunity set next run.

## 17. Disclaimer
This report is provided for informational and educational purposes only. It is not investment, legal, tax, or financial advice, and it is not a recommendation to buy, sell, short, hedge, or hold any security, fund, derivative, or index exposure. It does not take into account the specific investment objectives, financial situation, or particular needs of any recipient. Views are general in nature, may change without notice, and may not be suitable for every investor. Investing and shorting involve risk, including possible loss of principal.